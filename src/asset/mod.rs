...
pub fn run(cmd: AssetCmd) {
    match cmd {
        AssetCmd::Add { r#type, name, image, concept, keywords } => add(&r#type, &name, image, concept, keywords),
        ...
        AssetCmd::Confirm { name, image } => confirm(&name, &image),
    }
}

fn add(asset_type: &str, name: &str, image: Option<String>, concept: Option<String>, keywords: Option<String>) {
...
fn confirm(name: &str, image_path: &str) {
    match find_asset(name) {
        Some((path, mut meta)) => {
            ...
            if !src.exists() { eprintln!("이미지 파일을 찾을 수 없습니다: {}", image_path); std::process::exit(1); }

            let dir = crate::project::current_project_dir();
            let ext = src.extension().map(|e| e.to_string_lossy().to_string()).unwrap_or_else(|| "png".to_string());
            let confirmed_name = format!("{}.{}", name, ext);
            fs::copy(&src, dir.join("confirmed").join(&confirmed_name)).unwrap();
