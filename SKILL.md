...
**한글은 사용자 확인용, 영문은 미드저니 입력용.** 절대 섞지 않는다.
...
🎨 Midjourney 프롬프트 (영문, 실제 입력용):
(순수 영문 7섹션 디렉팅 시트. 이 블록을 그대로 복사해서 미드저니에 입력)
```

**aip prompt save 시 저장되는 건 영문 7섹션 시트만.** 한글 설명은 저장하지 않는다.

**7섹션 시트 내부에서도 한글 섞지 말 것.** `## Style`, `## Composition` 등 섹션 헤더와 내용 모두 영문.
미드저니는 한국어+영어 혼합을 받으면 퀄리티가 떨어진다.

사용자 확인용 한글 설명은 대화에서만 보여주고, 실제 저장/입력은 순수 영문만.
